...
#making a visulisation of all the countries and homicide cases
...
- How would you characterize the trend compared to the homicides above?
    answer: By looking at the visulizations above, you can see that the closer we get to "today", there are lesser homicides


##compairing the monarchs and homicides
```{r}
library(tidyverse)
```
...
```{r eval = TRUE}
Danish_Monarchs <- read_csv2("../DM03/data/Data-kongerækken-Simpel-Rigtige-Rigtig.csv")
...
Danish_Monarchs %>% 
 ggplot(aes( x= midyear , 
              y = Years_Rulled))+
  geom_smooth()+
  labs(title = "Reign Duration of Monarhcs", 
       x = "midyear", 
       y= "Years ruled")+
  theme_bw()
```
...
1) Plot: In the facetted plot above, move the legend from the current position on the side to below the facets, and label it "Country" instead of "Entity".
  see the two visulisation above 
...
3) Question: In <250 words articulate your answer on the basis of the data visualisations to the following question: are we more civilized today?
    answer (65 words): By looking at the visualization, you can see that the further up in time we get, the lesser homicides people commit. This might be because the system is getting stricter and the possibility of getting arrested (and getting a tougher sentence) is higher. So maybe is a mixture of us getting more civilized, but also the fear of getting caught that is the main reason. 
